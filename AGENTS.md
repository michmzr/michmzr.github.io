...
`DESIGN.md` is the foundation for the new CyberShu website design; the production implementation now uses its palette, typography and shared component styles. The business-card project supplies branding inspiration, the palette, typography and updated logo/wordmark only. Do not treat its composition, portrait placement, slogans, audience brief or print constraints as website requirements. Design page layouts and components around website content and user tasks. Explicit user instructions take priority. Keep tokens, prose and relevant specimens consistent when an accepted rule changes.
...
The generator synchronizes tokens, not natural-language requirements or production CSS. Update affected examples manually when prose rules change. Add a representative kitchen-sink specimen for each new production component or state, and keep its coverage table honest. Generic dialogs do not prove search coverage; the local demo form is not the real booking integration. The production specimen shares `_sass/cybershu/` styles and the navigation/search scripts; keep the four production partials valid plain CSS for standalone rendering.

## Repository and publication scope

Jekyll/TeXt styles enter through `assets/css/main.scss`; partials are in `_sass/`, with `_sass/custom.scss` composing `_sass/cybershu/`. Templates are in `_layouts/` and `_includes/`; navigation is in `_data/navigation.yml`. `consultations.md` uses the shared production stylesheet. Never edit generated `_site/` output.
