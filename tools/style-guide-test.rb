...
class StyleGuideTest < Minitest::Test
  def setup
    ...
    FileUtils.cp(File.expand_path('../assets/favicon.svg', __dir__), File.join(@root, 'assets'))
    FileUtils.mkdir_p(File.join(@root, '_includes'))
    FileUtils.cp_r(File.expand_path('../_includes/icons', __dir__), File.join(@root, '_includes'))
    FileUtils.cp(File.expand_path('../_includes/brand.html', __dir__), File.join(@root, '_includes'))
    FileUtils.mkdir_p(File.join(@root, 'assets/images'))
    FileUtils.cp_r(File.expand_path('../assets/images/logo', __dir__), File.join(@root, 'assets/images'))
